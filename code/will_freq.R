...
william_txt <- analyzer('./원문/william.txt')
grep("_VV$", william_txt, value=T)
grep("_V[^V]", william_txt, value=T)
grep("_V[^BH]", william_txt, value=T)
# 필요한 품사만 grep
...
filtered_tagged <- grep("_N|_V[^BH]|_JJ|_RB", william_txt, value=T)
#filtered_tagged <- grep("_NP", words_tagged, value=T)
filtered_tagged <- grep("_RB", william_txt, value=T)
