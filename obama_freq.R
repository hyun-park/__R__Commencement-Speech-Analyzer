...
grep("_VV$", obama_txt, value=T)
grep("_V[^V]", obama_txt, value=T)
grep("_V[^BH]", obama_txt, value=T)
# 필요한 품사만 grep
...
filtered_tagged <- grep("_N|_V[^BH]|_JJ|_RB", obama_txt, value=T)
#filtered_tagged <- grep("_NP", words_tagged, value=T)
filtered_tagged <- grep("_RB", obama_txt, value=T)
